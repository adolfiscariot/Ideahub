...
describe('AnalyticsService', () => {
  ...
  describe('convertResponse (via getMostVotedIdeas)', () => {
    it('should use response.status when success is absent', () => {
      const raw: RawMockResponse<MostVotedIdea[]> = { status: true, message: 'ok', data: [] };
    ...
    it('should use response.success when status is absent', () => {
      const raw = { success: true, message: 'ok', data: [] } as ApiResponse<MostVotedIdea[]>;
    ...
    it('should default success to false when both status and success are absent', () => {
      const raw: RawMockResponse<MostVotedIdea[]> = { message: 'fail', data: [] };
  ...
  describe('getMostVotedIdeas()', () => {
    it('should GET /analytics/most-voted', () => {
      const mockIdeas: MostVotedIdea[] = [
        { id: 1, title: 'AI Pipeline', proposedSolution: 'Automate', author: 'Alice', groupName: 'Tech', voteCount: 42, groupId: 10, isMember: true },
      ];
      const raw: ApiResponse<MostVotedIdea[]> = { success: true, message: 'ok', data: mockIdeas };
    ...
    it('should return empty data array when no ideas exist', () => {
      const raw: ApiResponse<MostVotedIdea[]> = { success: true, message: 'none', data: [] };
  ...
  describe('getTopContributors()', () => {
    it('should GET /analytics/top-contributors', () => {
      ...
      const raw: ApiResponse<TopContributor[]> = { success: true, message: 'ok', data: mockContributors };
  ...
  describe('getPromotedIdeas()', () => {
    it('should GET /analytics/promoted-ideas', () => {
      const mockIdeas: PromotedIdea[] = [
        { id: 1, title: 'Drone Delivery', proposedSolution: 'Drones', author: 'Dan', groupName: 'Logistics', promotedDate: '2024-01-01', projectId: 99 },
      ];
      const raw: ApiResponse<PromotedIdea[]> = { success: true, message: 'ok', data: mockIdeas };
  ...
  describe('getIdeaStatistics()', () => {
    it('should GET /analytics/idea-statistics and return stats', () => {
      const mockStats: IdeaStats = { total: 100, open: 60, promoted: 25, closed: 15 };
      const raw: ApiResponse<IdeaStats> = { success: true, message: 'ok', data: mockStats };
    ...
    it('should return zeros for an empty system', () => {
      const mockStats: IdeaStats = { total: 0, open: 0, promoted: 0, closed: 0 };
      const raw: ApiResponse<IdeaStats> = { success: true, message: 'ok', data: mockStats };
  ...
  describe('getGroupEngagement()', () => {
    it('should GET /analytics/group-engagement', () => {
      const mockEngagement: GroupEngagement[] = [
        { id: 1, name: 'Innovation Lab', ideaCount: 30, voteCount: 150, isMember: true },
      ];
      const raw: ApiResponse<GroupEngagement[]> = { success: true, message: 'ok', data: mockEngagement };
    ...
    it('should handle non-member groups correctly', () => {
      const mockEngagement: GroupEngagement[] = [
        { id: 2, name: 'External Group', ideaCount: 5, voteCount: 10, isMember: false },
      ];
      const raw: ApiResponse<GroupEngagement[]> = { success: true, message: 'ok', data: mockEngagement };
  ...
  describe('getPersonalStats()', () => {
    it('should GET /analytics/personal-stats', () => {
      const mockStats: PersonalStats = { ideasCreated: 5, votesCast: 20, projectsInvolved: 3, groupsCreated: 1 };
      const raw: ApiResponse<PersonalStats> = { success: true, message: 'ok', data: mockStats };
    ...
    it('should return zero stats for a new user', () => {
      const mockStats: PersonalStats = { ideasCreated: 0, votesCast: 0, projectsInvolved: 0, groupsCreated: 0 };
      const raw: ApiResponse<PersonalStats> = { success: true, message: 'ok', data: mockStats };
