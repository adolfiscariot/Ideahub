...
import { TimesheetDto, RelevantTask } from '../Interfaces/Timesheet/timesheet-interface';
...
describe('TimesheetService', () => {
  ...
  const api_url = `${environment.apiUrl}/timesheet`;
  ...
  describe('bulkLogWork()', () => {
    it('should POST /bulk-timesheets with projectId query param', () => {
      ...
      const mock_response: ApiResponse<{ createdIds: number[], invalidRows: unknown[] }> = {
        success: true,
        message: 'ok',
      ...
      const req = http_mock.expectOne(`${api_url}/bulk-timesheets?projectId=101`);
  ...
  describe('Timesheet Queries', () => {
    it('should GET /relevant-tasks?projectId={id}', () => {
      ...
      const req = http_mock.expectOne(`${api_url}/relevant-tasks?projectId=101`);
      expect(req.request.method).toBe('GET');
      req.flush(mock_response);
    });

    it('should GET /my-timesheets with optional projectId', () => {
      const mock_response: ApiResponse<TimesheetDto[]> = { success: true, message: 'ok', data: [] };
      ...
      http_mock.expectOne(`${api_url}/my-timesheets?projectId=101`).flush(mock_response);
    });

    it('should GET /project-team?projectId={id}', () => {
       const mock_response: ApiResponse<{ id: string, name: string }[]> = { success: true, message: 'ok', data: [] };
       service.getProjectTeam(101).subscribe();
       http_mock.expectOne(`${api_url}/project-team?projectId=101`).flush(mock_response);
    });
  });
