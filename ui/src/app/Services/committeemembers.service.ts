...
export class CommitteeMembersService {
    ...
    getCommitteeMembers(): Observable<ApiResponse<UserRecord[]>> {
        return this.http.get<ApiResponse<UserRecord[]>>(this.apiUrl).pipe(
            map(response => this.convertResponse<UserRecord[]>(response))
        );
    }

    getAllUsers(): Observable<ApiResponse<UserRecord[]>> {
        return this.http.get<ApiResponse<UserRecord[]>>(`${this.apiUrl}/users`).pipe(
            map(response => this.convertResponse<UserRecord[]>(response))
        );
    }

    addCommitteeMember(email: string): Observable<ApiResponse<void>> {
        return this.http.post<ApiResponse<void>>(`${this.apiUrl}/add/${email}`, {}).pipe(
